...
with open('2.txt') as f:
    for password_line in f.readlines():
        ...
        pos1_has_letter = True if password[int(include_position) - 1] == letter else False
        pos2_has_letter = True if password[int(exclude_position) - 1] == letter else False
        if (pos1_has_letter and not pos2_has_letter) or (not pos1_has_letter and pos2_has_letter):
            num_valid += 1
print(num_valid)
